...
module.exports = {
  ...
  add: async (values) => {
    ...
    return Application.updateRelations({ _id: entry.id, values: relations });
  },
  ...
  search: async (params) => {
    ...
    const $or = Object.keys(Application.attributes).reduce((acc, curr) => {
      switch (Application.attributes[curr].type) {
        case 'integer':
        case 'float':
        case 'decimal':
